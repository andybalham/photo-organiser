...
Phases 1–4 complete + Special Dates + Date Ranges + Shareable Dates feature. Solution scaffolded, UI built, FileScanner service implemented (121 unit tests passing). Phase 4 wired: Analyse button runs ScanAsync async, guards source==dest and dest-inside-source, populates summary label and colour-coded log, enables Start Copy only when actionable work exists. Special Dates: SpecialDate model, ISpecialDateService/SpecialDateService (JSON persistence in %LOCALAPPDATA%\PhotoOrganiser\special_dates.json), injected into FileScanner, Special Dates tab in MainForm with DataGridView + Add/Delete. Folder name sanitisation: `FileNameHelper.SanitiseFolderName` strips illegal path chars (replaces with `-`) applied to special date and date range names when building destination folder path; original name preserved in JSON. Date Ranges: DateRange model (Name, StartDate, EndDate as DateOnly), persisted to %LOCALAPPDATA%\PhotoOrganiser\date_ranges.json, MatchRange checks file date falls within range (inclusive, supports cross-month/year), checked after Special Dates (Special Date wins on overlap), separate Date Ranges tab in MainForm. Shareable Dates: IShareableDatesService/ShareableDatesService combines both collections into a single *.dates.json file; linked file path persisted in %LOCALAPPDATA%\PhotoOrganiser\settings.json; auto-loaded on startup (replaces both collections); auto-saved on every Add/Delete; Link File…/Save File…/Unlink/Open Folder buttons + status label in panel below Destination field. Save File… exports current data to any path without changing linked file. Phase 5+ not yet implemented.
